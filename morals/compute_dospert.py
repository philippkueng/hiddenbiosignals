...
def load_and_preprocess_data(file_path):
    ...
    targets = df[['financial_likelihood_dospert','recreational_likelihood_dospert','recreational_perceived_dospert']].astype(float).values
...
def train_model(model, train_loader, val_loader, device, num_epochs=100, early_stopping_patience=20):
    ...
    target_labels = ['financial_likelihood_dospert','recreational_likelihood_dospert','recreational_perceived_dospert']
...
def export_predictions(model, data_loader, input_texts, auxiliary_features, original_df, file_path, device):
    ...
    pred_df = pd.DataFrame(all_preds, columns=[ 'Pred_financial_likelihood_dospert','Pred_recreational_likelihood_dospert','Pred_recreational_perceived_dospert'])
    true_df = pd.DataFrame(all_labels, columns=['True_financial_likelihood_dospert','True_recreational_likelihood_dospert','True_recreational_perceived_dospert'])
...
def main():
    # Configuration
    file_path = 'updated_aggregated_body_aug_anno_Dospert.xlsx'
    # excel file with dospert training data of structure
    # user_id	mail_user	age_user	gender_user	ethical_likelihood_dospert	financial_likelihood_dospert	health_likelihood_dospert	recreational_likelihood_dospert	social_likelihood_dospert	total_likelihood_dospert	ethical_perceived_dospert	financial_perceived_dospert	health_perceived_dospert	recreational_perceived_dospert	social_perceived_dospert	total_perceived_dospert	body	is_augmented	Text	num_nouns	num_verbs	num_adjectives	num_adverbs	first_person_pronoun_score	second_person_pronoun_score	third_person_pronoun_score	sentiment_score	sentiment_label
    # 1319	person@mail.com		0	1	3.166666667	1.666666667	4.833333333	6.666666667	3.466666667	5.166666667	5.5	5	4.5	2.5	5	text	7	2	2	0	0	0	0	-0.97048831	negative
  ...
  transformer_name = "sentence-transformers/all-MiniLM-L6-v2"
    ...
    if len(test_loader) == 0:
        ...
        export_predictions(
            ...
            "test_predictions_Dospert_aug2.csv",
            device
        )
